...
class PrestaBlog extends Module implements WidgetInterface
{
    ...
    */
    ...
    {
        foreach (['AdminPrestaBlog'] as $tab_name) {
            $id_tab = (int) Tab::getIdFromClassName($tab_name);
            if ($id_tab) {
                $tab = new Tab($id_tab);
                $tab->delete();
...
';
  ...
  color: #fff;
            ...
            $color_title = '#blog_list_1-7 .block_bas h3 a, #blog_list_1-7 .block_bas .h3title a {
 ...
 */
 ...
 if (!count($errors))
    ...
    private function displayNavConfiguration()
    {
      ...
      <a href="' . self::prestablogUrl([]) . '" target="_blank">
    ...
    . (int) Configuration::get($this->name . '_lastnews_limit') . ' ' . $this->l('latest news in') . ' ' . Language::getIsoById((int)($this->context->language->id)) . '
    </p>' . "\n";
        ...
        if (count($liste_news) > 0) {
            for ($i = 0; isset($liste_news[$i]); $i++) {
                if ($i <= (int) Configuration::get($this->name . '_lastnews_limit') + 1) {
                    ...
                    for ($j = 0; isset($lang_liste_news[$j]); $j++) {
...
*/
    ...
    private function displayAddSlide()
    {
        ...
        foreach ($languages as $language) {
            ...
            $html_libre .= ' ';
            $html_libre .= ' />
                        <img src="../img/l/' . (int) $lid . '.jpg" class="pointer indent-right" alt="' . $language['name'] . '" title="' . $language['name'] . '" />';
        }
        ...
        $html_libre .= '
                      <div id="image">
                      <input type="file" name="load_img_slide" id="load_img_slide" value="" />
                      </div>';
        $this->html_out .= $this->displayFormLibre(
            'col-lg-2',
            $this->l('Your slide'),
    ...
    private function displayEditSlide()
    {
        ...
        $this->html_out .= '
                      ...
                      <div class="col-lg-7">
                      <img class="item" src="' . self::imgPathBO() . self::getT() . '/slider/' . $id_slide . '.jpg" />
                      </div>
                      </div>';
        ...
        $html_libre .= '
                      <div id="url_associate">
                      <input type="text" name="url_associate" id="url_associate" value="' . $url_associate . '" />
                      </div>';
        ...
        $html_libre .= '
                      <div id="id_slide">
                      <input type="text" name="id_slide" id="id_slide" value="' . $id_slide . '" />
                      </div>';
    ...
    private function displaySlideSystem()
    {
        if (Tools::getValue('success') == 'su') {
            if (!Tools::getIsset('reload')) {
                Tools::redirect('Location:' . $this->confpath . '&configSlide&success=su&reload' . '');
            }
        ...
        if (Tools::getValue('error') && Tools::getValue('error') == 'pos') {
            if (!Tools::getIsset('reload')) {
                Tools::redirect('Location:' . $this->confpath . '&configSlide&error=pos&reload' . '');
            }
            ...
            if (!Tools::getIsset('reload')) {
                Tools::redirect('Location:' . $this->confpath . '&configSlide&errorslide=la&reload' . '');
            }
            ...
            if (!Tools::getIsset('reload')) {
                Tools::redirect('Location:' . $this->confpath . '&configSlide&error=size&reload' . '');
            }
        ...
        if (Tools::getValue('languesup')) {
            $slider = SliderClass::getListSlider((int) $this->context->shop->id, Tools::getValue('languesup'));
        } else {
            $slider = SliderClass::getListSlider((int) $this->context->shop->id);
        }
        $this->html_out .= '<div class="clearfix"></div>';
        if (Tools::getValue('languesup')) {
    ...
    private function displayListeNews()
    {
        ...
        if (count($liste) > 0) {
            foreach ($liste as $value) {
                ...
                for ($i = 0; isset($lang_liste_news[$i]); $i++) {
                ...
                if (Tools::getValue('languesup') && isset($langue) && $langue == true) {
                    ...
                    foreach ($lang_liste_news as $val_langue) {
                        if (count($languages) >= 1 && array_key_exists((int) $val_langue, $languages_shop) && Tools::getValue('languesup') == (int) $val_langue) {
                            ...
                            $this->html_out .= '
                                    ...
                                    <a target="_blank" href="' . PrestaBlog::prestablogUrl([
                    ...
                    foreach ($lang_liste_news as $val_langue) {
                        if (count($languages) >= 1 && array_key_exists((int) $val_langue, $languages_shop)) {
                            $this->html_out .= '<img src="../img/l/' . (int) $val_langue . '.jpg" />';
                            $this->html_out .= '<a target="_blank" href="' . PrestaBlog::prestablogUrl([
                                'id' => (int) $news_tempo->id,
                                'seo' => $news_tempo->link_rewrite[(int) $val_langue],
                                'titre' => $news_tempo->title[(int) $val_langue],
                                'id_lang' => (int) $val_langue,
...
//}
   //     return true;
    ...
    {
        $languages = Language::getLanguages(true);
        $div_lang_name = 'meta_title¤meta_description¤titre_h1';
        ...
        foreach ($languages as $language) {
            $lid = (int) $language['id_lang'];
            $html_libre .= '
                  <div id="meta_title_' . $lid . '" style="display: ' . ($lid == $this->langue_default_store ? 'block' : 'none') . ';">
                  <input type="text" name="meta_title_' . $lid . '" id="meta_title_' . $lid . '" value="' . Configuration::get($this->name . '_titlepageblog', (int) $lid) . '" />
                  </div>';
        }
        $this->html_out .= $this->displayFormLibre(
            'col-lg-2',
        ...
        foreach ($languages as $language) {
            $lid = (int) $language['id_lang'];
            $html_libre .= '
          <div id="meta_description_' . $lid . '" style="display: ' . ($lid == $this->langue_default_store ? 'block' : 'none') . ';">
          <input type="text" name="meta_description_' . $lid . '" id="meta_description_' . $lid . '" value="' . Configuration::get($this->name . '_descpageblog', (int) $lid) . '" />
          </div>';
        }
        $this->html_out .= $this->displayFormLibre(
            'col-lg-2',
        ...
        foreach ($languages as $language) {
            $lid = (int) $language['id_lang'];
            $html_libre .= '
            <div id="titre_h1_' . $lid . '" style="display: ' . ($lid == $this->langue_default_store ? 'block' : 'none') . ';">
            <input type="text" name="titre_h1_' . $lid . '" id="titre_h1_' . $lid . '" value="' . Configuration::get($this->name . '_h1pageblog', (int) $lid) . '" />
            </div>';
        }
        $this->html_out .= $this->displayFormLibre(
            'col-lg-2',
            ...
            $html_libre,
            'col-lg-7',
            $this->displayFlagsFor('titre_h1', $div_lang_name)
        );
        //***********************************************************
    ...
    private function displayConfBlocs()
    {
        ...
        $this->html_out .= $this->displayFormSelect(
            ...
            ['desc' => $this->l('Desc'), 'asc' => $this->l('Asc')],
            null,
            'col-lg-5'
        );
        $this->html_out .= $this->displayFormEnableItemConfiguration(
...
*/
    ...
    private function displayFormNews()
    {
        ...
        foreach ($languages as $language) {
            ...
            $this->html_out .= '
                                                              if ($("#slink_rewrite_' . $lid . '").val() == \'\') {
                                                                ...
                                                                $("#prestablog_control").html("' . $this->l('Disable this rewrite') . '");
                                                              }
        ...
        $this->html_out .= '$("#productLinkSearch").bind("keydown", function() {
        ...
        if (Tools::getValue('idN')) {
            ...
            foreach ($lang_liste_news as $val_langue) {
                if (count($languages) >= 1 && array_key_exists((int) $val_langue, $languages_shop)) {
                    $html_libre .= '<a target="_blank" href="' . PrestaBlog::prestablogUrl([
                        'id' => (int) $news->id,
        ...
        $urlrw = $this->l('Url Rewrite') . '<br/><a href="#" id="prestablog_control" />';
    ...
    private function displayFormSubBlocksFront()
    {
                  ...
                  RetourLangueCheckUp(selectedL, this.value, ' . $dl . '), \'\');
                                ...
                                $("#submitForm").click(function( event ) {
                                  ...
                                  $("input[name=\'languesup[]\']:checked").each(function() {
                                   ...
                                   function changeTheLanguage(title, divLangName, id_lang, iso) {
                                      $("#imgCatLang").attr("src", "../img/l/" + id_lang + ".jpg"); return changeLanguage(title, divLangName, id_lang, iso);
                                    }
        ...
        //***********************************************************
        $html_libre = '<span id="check_lang_prestablog">';
        $checkmelang = '<input type="checkbox" name="checkmelang" class="noborder"
                               onclick="checkDelBoxes(this.form, \'languesup[]\', this.checked)" />
                       ' . $this->l('All') . ' | ';
